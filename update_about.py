...
# I will append manually since JSON parsing inside a heavily formatted raw file is hard.
additions = {
    "en": {
...
for lang, data in additions.items():
    pattern = rf'("{lang}": {{\n)'
    payload = f'        "btn_about_project": "{data["btn_about_project"]}",\n        "about_text": "{data["about_text"]}",\n'
    content = re.sub(pattern, rf'\1{payload}', content)
